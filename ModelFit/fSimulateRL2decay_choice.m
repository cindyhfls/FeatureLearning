function rl2 = fSimulateRL2decay_choice(xpar, sesdata)
%
...
alpha_rewShape = xpar([NparamBasic+1:NparamBasic+nChops]) ;
if sesdata.flagUnr==1
    alpha_unrColor = xpar([NparamBasic+1+1*nChops:NparamBasic+2*nChops]) ;
    alpha_unrShape = xpar([NparamBasic+1+1*nChops:NparamBasic+2*nChops]) ;
else
...
inputReward     = sesdata.input.inputReward ;
...
for cnt_trial=ntrials
    ...
    choice      = ( pChoiceR > rand ) + 1 ;
    if inputReward(choice, cnt_trial)
       correct = 1 ;
    else
       correct = 0 ;
    end
    ...
    V(:,cnt_trial) = v ;
    rl2.correct(cnt_trial) = correct ;
    rl2.choice(cnt_trial)  = choice ;
